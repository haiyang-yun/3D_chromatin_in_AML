...
my $control = $ARGV[2]; # control file in .BED format if present
#chomp $control;
...
my $name2 = $name.".NoModel_p".$val;
$cmd = "macs2 callpeak -t $inputFile -c $control -g $gSize -n $name2 -f BED -p $val --nomodel  --bdg ";
print "\n\nRunning command: $cmd\n\n";
system($cmd);
